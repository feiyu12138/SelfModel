...
class VisModelingModel(pl.LightningModule):
    ...
    def __build_model(self):
        ...
        if self.hparams.model_name == 'state-condition-modulated':
            self.model = StateConditionModulatedModel(in_channels=int(3+self.hparams.dof), out_channels=1, hidden_features=256)
    ...
    def siren_sdf_loss(self, model_output, gt):
        ...
        gt_sdf = gt_sdf.reshape(-1, 1).contiguous()
        gt_normals = gt_normals.reshape(-1, 3).contiguous()
        ...
        loss = loss_dict['sdf'] + loss_dict['inter']  + loss_dict['grad_constraint'] + loss_dict['normal_constraint']
        return loss
    
    def train_forward(self, data):
        if self.hparams.model_name == 'state-condition' or self.hparams.model_name == 'state-condition-modulated':
        ...
        if self.hparams.model_name == 'state-condition' or self.hparams.model_name == 'state-condition-modulated':
            if stage == 'fit':
                if self.hparams.loss_type == 'siren_sdf':
                    ...
                    self.val_dataset = MultipleModel(flag='val',
                                                        seed=self.hparams.seed,
                                                        pointcloud_folder=self.hparams.data_filepath,
                                                        on_surface_points=self.hparams.train_batch)
            if stage == 'test':
