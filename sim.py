...
class ArmPybulletSim(object):
    def __init__(self, gui_enabled, num_cam,start_step=0):
        ...
        self.save_mesh_folder = "/ccvl/net/ccvl15/luoxin/saved_meshes"
        if not os.path.exists(self.save_mesh_folder):
    ...
    def save_robot_state(self):
        with open(os.path.join(self.save_mesh_folder, f'robot_state_{self.start_step}.json'), 'w') as file:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--end_step', type=int, default=10000)
    args = parser.parse_args()
    env = ArmPybulletSim(gui_enabled=False, num_cam=5,start_step=args.start_step)
    N = args.end_step
    while True:
